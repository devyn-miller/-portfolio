...
```sh {"id":"01HQVAY3CNKTRRCWCTYS06N75P"}
...
> __Note__: `SENDGRID_API_KEY` - Create an API key from "Settings" -> "API Keys" with "Restricted Access" to only "Mail Send"
...
```bash {"id":"01HQVAY3CNKTRRCWCTYS1E5HHM"}
npm run dev
```
...
```sh {"id":"01HQVAY3CNKTRRCWCTYWERHT8A"}
SENDGRID_API_KEY=XXXXXXXX
NEXT_PUBLIC_FIREBASE_DATABASE_URL=XXXXXXXXXX
MAIL_FROM=YOUR_MAIL_ID
MAIL_TO=YOUR_MAIL_ID
